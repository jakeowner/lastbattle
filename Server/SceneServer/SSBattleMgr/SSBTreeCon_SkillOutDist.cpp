...
namespace SceneServer{
	...
	BOOLEAN	CSSBTreeCon_SkillOutDist::Travel(CSSAI_HeroRobot* pAI,CSSBTreeNode *&pActionNode,vector<SBTreeResult> *rstVec){
		...
		if(m_parameters[0] == 1){
			...
			return FALSE == pHero->GetCurPos().CanWatch(dist, pTar->GetCurPos());
		}
			...
			for(auto iter = pSkillVec->begin(); iter != pSkillVec->end(); ){
				...
				if(FALSE == pHero->GetCurPos().CanWatch(dist, pTar->GetCurPos())){
					iter++;
				}
				else{
